    ...
    with torch.no_grad():
        for model in client_models:
            for (client_p, global_p) in zip(model.parameters(), global_model.parameters()):
                ...
                global_p.grad.copy_(global_p.grad + client_p.grad)
        for global_p in global_model.parameters():
            global_p.grad.copy_(global_p.grad / nb_devices)
        ...
        for _ in range(iter_steps):
            ...
            # Down-compression step
                ...
                with torch.no_grad():
                    ...
                    for p, global_p in zip(model.parameters(), preserved_model.parameters()):
                    ...
                    if parameters.use_down_memory:
                        for zipped_omega in model.parameters():
                            param_state = optimizers[w_id].state[zipped_omega]
                            if down_learning_rate_name not in param_state:
                                if parameters.down_compression_model.level != 0:
                                    param_state[down_learning_rate_name] = 1 / (
                                            2 * (parameters.down_compression_model.__compute_omega_c__(
                                        zipped_omega) + 1))
                                else:
                                    param_state[down_learning_rate_name] = 0
                            dezipped_omega = zipped_omega + param_state[down_memory_name]
                            param_state[down_memory_name] += zipped_omega.mul(
            ...
            for w_id in active_worker:
                ...
                optimizers[w_id].zero_grad()
                output = client_models[w_id](data).to(device)
                if torch.isnan(output).any():
                ...
                loss = criterion(output, target)
                loss.backward()
                optimizers[w_id].step_local_global(w_id)

            server_aggregate_gradients(preserved_model, client_models, device)

            if parameters.non_degraded:
                with torch.no_grad():
                    # Updating the model
                    for global_p in preserved_model.parameters():
                        # param_state = optimizers[w_id].state[p]
                        update_model = global_p - global_p.grad.mul(parameters.optimal_step_size)
                        global_p.copy_(update_model)
            else:
                with torch.no_grad():
                    ...
                    for global_p in preserved_model.parameters():
                        # param_state = optimizers[w_id].state[p]
                        update_model = global_p - parameters.down_compression_model.compress(global_p.grad.mul(parameters.optimal_step_size))
                        global_p.copy_(update_model)
