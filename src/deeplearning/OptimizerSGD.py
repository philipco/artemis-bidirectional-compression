...
class SGDArtemis(Optimizer):
    ...
    def __init__(self, nn_model_params, parameters: DLParameters, dampening=0,
        ...
        super(SGDArtemis, self).__init__(nn_model_params, defaults)
    ...
    def __setstate__(self, state):
        super(SGDArtemis, self).__setstate__(state)
        ...
        for group in self.param_groups:
            ...
            for p in group['params']:
                ...
                if up_learning_rate_name not in param_state:
                    param_state[up_learning_rate_name] = self.parameters.up_compression_model.get_learning_rate(
                        loc_grad)
                ...
                if self.parameters.use_up_memory:
                    ...
                    param_state[up_local_memory_name] = param_state[up_local_memory_name] + delta.mul(
                        param_state[up_learning_rate_name]).detach()
                else:
                    grad = delta

                if not self.parameters.use_up_memory:
                    assert up_local_memory_name not in param_state, "Up memory should not be in parameters' state."
